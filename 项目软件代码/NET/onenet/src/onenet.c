...
#define PROID			"8OCvDyLab8"

#define ACCESS_KEY		"TXpUZHhIT2NqOGdIbFU3NG03WGVNVUdjSVFWS09MU2M="

#define DEVICE_NAME		"w1"
...
extern u8 temp,humi,distance,light;
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"buzzer\":{\"value\":%s},", buzzer_info.Buzzer_Status ? "true" : "false");
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"motor_status\":{\"value\":%s},", motor_info.Motor_Status ? "true" : "false");
	strcat(buf, text);
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				cJSON *motor_json=cJSON_GetObjectItem(params_json,"motor_status");
				if(motor_json!=NULL)
				{
					if(motor_json->type==cJSON_True)
					{
						Motor_Set(MOTOR_ON);
					}else
					{
				...
				if(buzzer_json!=NULL)
				{
					if(buzzer_json->type==cJSON_True)
					{
